...
public protocol KolodaViewDelegate: class {
    ...
    func koloda(_ koloda: KolodaView, firstCardDidLayout index:Int, view:UIView?)
...
open class KolodaView: UIView, DraggableCardDelegate {
    ...
    public var topDismissPoint:CGPoint?
    ...
    private func layoutCard(_ card: DraggableCardView, at index: Int) {
        if index == 0 {
            ...
            delegate?.koloda(self, firstCardDidLayout: index, view:card.contentView)
        } else {
    ...
    private func loadNextCard() {
        ...
        lastCard.topDismissPoint = topDismissPoint
        let scale = cardParameters.scale
